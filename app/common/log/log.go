...
func New(opts *Options) *zapLogger {
	...
	if opts.EnableColor && opts.Format == ConsoleFormat {
		encodeLevel = zapcore.CapitalColorLevelEncoder
	}

	lowHook := lumberjack.Logger{
		Filename:   fmt.Sprintf("%s/%s.log", strings.TrimRight(opts.OutputPath, "/"), opts.Name), // 日志文件路径
		MaxSize:    opts.HookOptions.MaxSize,
		MaxBackups: opts.HookOptions.MaxBackups,
		MaxAge:     opts.HookOptions.MaxAge,
		Compress:   opts.HookOptions.Compress,
		LocalTime:  true,
	}
	highHook := lumberjack.Logger{
		Filename:   fmt.Sprintf("%s/%s_error.log", strings.TrimRight(opts.OutputPath, "/"), opts.Name), // 日志文件路径
		MaxSize:    opts.HookOptions.MaxSize,
		MaxBackups: opts.HookOptions.MaxBackups,
		MaxAge:     opts.HookOptions.MaxAge,
		Compress:   opts.HookOptions.Compress,
	...
	//stack := zap.AddStacktrace(zapLevel)
	filed := zap.Fields(zap.String("serviceName", opts.SName), zap.String("appName", opts.Name))
	...
	if strings.EqualFold(zapLevel.String(), "debug") {
	...
	var lowWriteSyncer, highWriteSyncer zapcore.WriteSyncer

	var lowWs []zapcore.WriteSyncer
	var highWs []zapcore.WriteSyncer

	lowWs = append(lowWs, zapcore.AddSync(&lowHook))
	highWs = append(highWs, zapcore.AddSync(&highHook))

	if opts.LogStdout || opts.Development {
		lowWs = append(lowWs, zapcore.AddSync(os.Stdout))
		highWs = append(highWs, zapcore.AddSync(os.Stdout))
	}
	lowWriteSyncer = zapcore.NewMultiWriteSyncer(lowWs...)
	highWriteSyncer = zapcore.NewMultiWriteSyncer(highWs...)
	if opts.Development {
	...
	if !opts.DisableStacktrace {
		st := zap.AddStacktrace(zap.DebugLevel)
		options = append(options, st)
	}

	lowCore := zapcore.NewCore(
		logEncoder,     // 编码器配置
		lowWriteSyncer, // 打印到控制台和文件
		lowPriority,    // 日志级别
	)
	highCore := zapcore.NewCore(
		logEncoder,      // 编码器配置
		highWriteSyncer, // 打印到控制台和文件
		highPriority,    // 日志级别
	)
	l := zap.New(zapcore.NewTee(lowCore, highCore), options...)
