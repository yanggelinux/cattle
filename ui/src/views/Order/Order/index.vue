<template>
  <div class="order-container app-container">
    <div class="order-header app-header">
      <el-row>
        <el-col :span="12">
        ...
        >
          ...
          >
            <template #default="scope">
              ...
              <span v-else-if="item === 'orderType'">
                {{ newOrderTypeMapping[scope.row[item]] }}
              </span>
...
import { processStatusMapping, newOrderTypeMapping } from '@/utils/constant'
...
// 打开弹窗
async function handleOpenDialog(opt: string, rowData?: OrderResult) {
  visible.value = true
  action.value = opt
  if (opt === 'update') {
...
function handleDelete(rowData: OrderResult) {
  ElMessageBox.confirm(`确定要删除${rowData?.name}吗？`, '提示', {
    ...
    .then(async () => {
      ...
      try {
        ...
        const resp: AxiosResponse = await OrderAPI.delete(id)
        const status = resp.data.status
        const msg = resp.data.msg
        if (status === 200) {
...
function handleCopy(row: OrderResult) {
  ElMessageBox.confirm(`确定要复制工单${row?.name}吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
    lockScroll: false,
    ...
    .then(async () => {
      ...
      try {
        ...
        const resp: AxiosResponse = await OrderAPI.copy(id)
        const status = resp.data.status
        const msg = resp.data.msg
        if (status === 200) {
...
function handleToField(order: OrderResult) {
  router.push({
    path: `/order/field/${order.id}`,
    query: {
      orderID: order?.id,
